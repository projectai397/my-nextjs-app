...
const ChildUserList: React.FC<UserListProps> = ({
  authenticated,
  apiClient,
  onUserSelect,
  initialUser,
    ...
    if (onNavigate) {
      ...
      if (state?.item) {
        const q = new URLSearchParams({
          item: btoa(JSON.stringify(state.item)),
        });
  ...
  const fetchList = async (overrideUserId?: string) => {
    try {
      setLoading(true);
      const parentUserId = overrideUserId ?? initialUser?.userId ?? "";
      const basePayload = {
  ...
  // initialize defaults (roleId), balance, roles
  useEffect(() => {
    ...
    fetchUserBalance();
    fetchRoles(initialUser?.userId);
  }, [authenticated.role, initialUser?.userId]);

  // Unified fetch: runs when (userId / filters / page) change with debounce
  useEffect(() => {
    const uid = initialUser?.userId ?? "";
    const t = setTimeout(() => {
      fetchRoles(uid);
      if (uid) {
        // Only fetch if parent user ID exists
        fetchList(uid);
      }
    }, 350);
    ...
    <div className="h-full">
      ...
      <Card className="border rounded-2xl bg-[#141a22] border-[#2a3038] mt-4">
        <CardHeader className="pb-2">
          <CardTitle className="text-base text-[#fcd535]">Users</CardTitle>
        </CardHeader>
                      ...
                      <TableCell className="text-left">
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handleCopy(u.ipAddress, "Copied Successfully!")
                            }
                          >
                            Copy
                          </Button>
                      ...
                      <TableCell className="text-left">
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handleCopy(u.deviceId, "Copied Successfully!")
                            }
                          >
                            Copy
                          </Button>
